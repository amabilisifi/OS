...
function InitFirstProcess()
  ...
  var thread: ptr to Thread
    ...
    thread.Init("UserLevelProgramThread")
    thread.Fork(StartUserProcess, 0)

endFunction
-------------------StartUserProcess-----------------------------------

function StartUserProcess(processNum: int)
  var
  processControlBlock:ptr to ProcessControlBlock
	openedFile:ptr to OpenFile
	initPC:int
	oldStatus:int
	initUserStackTop:int
	initSystemStackTop:ptr to int
  -- first we want to allocate new process controller block and  allocate that to currentThread
    processControlBlock=processManager.GetANewProcess()
    processControlBlock.myThread=currentThread
    currentThread.myProcess=processControlBlock

  -- now we want to open our wxwcutable file
    openedFile=fileManager.Open("TestProgram1")
    if openedFile==null
      FatalError("Could Not Open TestProgram1..!")
    endIf
    -- we need to create a logical address space and load executable file into it 
    initPC=openedFile.LoadExecutable(&processControlBlock.addrSpace)

  fileManager.Close(openedFile)

  -- now we want to initial value of user level stack 
    initUserStackTop=(processControlBlock.addrSpace.numberOfPages* PAGE_SIZE)
    initSystemStackTop= & currentThread.systemStack[SYSTEM_STACK_SIZE-1]

  -- now we disable interrupts and turns a threads to user level thread
    oldStatus=SetInterruptsTo(DISABLED)
    processControlBlock.addrSpace.SetToThisPageTable()
...
-----------------------------  Handle_Sys_Exit  ---------------------------------
  ...
  function Handle_Sys_Exit (returnStatus: int)
      -- NOT IMPLEMENTED
      print("Handle_Sys_Exit is happening\n")
      print ("returnStatus is:")
...
-----------------------------  Handle_Sys_Yield  ---------------------------------
  ...
  function Handle_Sys_Yield ()
      -- NOT IMPLEMENTED
      print("Handle_Sys_Yeild is happening ... \n")
    endFunction
...
-----------------------------  Handle_Sys_Fork  ---------------------------------
  ...
  function Handle_Sys_Fork () returns int
      -- NOT IMPLEMENTED
      print("Handle_Sys_Fork is happenning\n")
      return 1000
    endFunction
...
-----------------------------  Handle_Sys_Join  ---------------------------------
  ...
  function Handle_Sys_Join (processID: int) returns int
      -- NOT IMPLEMENTED
      print("Handle_Sys_Join is happening\n")
      print("processID is:")
      printInt(processID)
      nl()
      return 2000
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
  ...
  function Handle_Sys_Exec (filename: ptr to array of char) returns int
      ...
      var 
        ...
        -- first we transfer the filename string to system space
        ...
        -- then we open the executable file
        
        openedFile = fileManager.Open (&strBuffer)
        if openedFile == null
          return -1
        endIf

        -- then we need to create the new logical address space for the executable file
        
        newAddrSpace.Init()
        initPC = openedFile.LoadExecutable(&newAddrSpace)
        if initPC < 0
          print ("In Handle_Sys_Exec, is not able to get newAddrSpace\n")
          return -1
        endIf
        ...
        -- closing the executable file
        
        fileManager.Close (openedFile)

        -- computing the user-level stack value, initUserStackTop -> just like init we do the same
        
        initUserStackTop = (currentThread.myProcess.addrSpace.numberOfPages * PAGE_SIZE)

        -- we should make system stack top to full size
        
        initSystemStackTop = & currentThread.systemStack[SYSTEM_STACK_SIZE-1]
        oldStatus = SetInterruptsTo (DISABLED)
        currentThread.isUserThread = true

        -- at the end we make this thread a user thread so it can execute the executable file that we ae trying to
...
-----------------------------  Handle_Sys_Create  ---------------------------------
  ...
  -- we want to create a new file on the disk via this function :  
  ...
  function Handle_Sys_Open (filename: ptr to array of char) returns int
      -- NOT IMPLEMENTED
        ...
        var
            ...
            print ("Handle_Sys_Open is haappening\n")
...
-----------------------------  Handle_Sys_Read  ---------------------------------
  ...
  function Handle_Sys_Read (fileDesc: int, buffer: ptr to char, sizeInBytes: int) returns int
      -- NOT IMPLEMENTED
      -- This function reads that many bytes(passed to this function by callee)  from the current position in the file and places them in memory.
  -- The current file position will be advanced by the number of bytes transferred

      print ("Handle_Sys_Read is happening \n")
      print ("fileDesc = ")
      printInt (fileDesc)
      print("\n")
...
-----------------------------  Handle_Sys_Write  ---------------------------------
  ...
  function Handle_Sys_Write (fileDesc: int, buffer: ptr to char, sizeInBytes: int) returns int
      -- NOT IMPLEMENTED
       -- This function writes that many bytes(passed to this function by callee)  from the current position in the file and places them in memory.
  -- The current file position will be advanced by the number of bytes transferred

      print ("Handle_Sys_Write is happening \n")
      print ("fileDesc = ")
      printInt (fileDesc)
      print ("\n")
      print ("virtual address of buffer = ")
...
-----------------------------  Handle_Sys_Seek  ---------------------------------
  ...
  function Handle_Sys_Seek (fileDesc: int, newCurrentPos: int) returns int
      -- This function sets the current position in the file to the given value and returns the new current position.

      print ("Handle_Sys_Seek is happening\n")
      print ("fileDesc = ")
      printInt (fileDesc)
      print ("\n")
      print ("newCurrentPos = ")
...
-----------------------------  Handle_Sys_Close  ---------------------------------
  ...
  function Handle_Sys_Close (fileDesc: int)
      -- NOT IMPLEMENTED
      print ("Handle_Sys_Close is happening")
      print ("\n")
      print ("fileDesc = ")
      printInt (fileDesc)
      nl ()
